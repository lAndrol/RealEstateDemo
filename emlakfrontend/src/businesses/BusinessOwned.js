...
export default function BusinessOwned() {
  ...
  const{squareMeter,price,numberOfRooms,floor,numberOfFloors,customer,business,heating,estateType} = estate
    ...
    const loadEstates=async()=>{
        const owner = await axios.get(`http://localhost:8080/business/${businessId}`)
        const result=await axios.post(`http://localhost:8080/estate/businessowned`,owner.data);
...

    {
      displayEstateList.map((estate,index)=>(
        ...
        <td>
          ...
          <Link className="btn btn--outline-primary mx-2" to={`/editestate/${estate.estateId}`}>Değiştir</Link>
          <button className="btn btn-danger mx-2" onClick={()=>deleteEstate(estate.estateId)}>Sil</button>
        </td>
      </tr>
      ))
    }
